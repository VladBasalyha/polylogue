<template>
	<div id="CreatePost" class="fixed z-50 bottom-0 h-full w-full overflow-hidden">
		<div class="bg-gray-900 h-full w-full text-white overflow-auto">
			<div class="flex items-center justify-start py-4 max-w-[500px] mx-auto border-b border-b-gray-700 text-gray-400">
				<button
					@click="
				...
				<div class="py-2 w-full">
					<div class="flex items-center">
						<div v-if="user" class="flex items-center text-white">
							...
							<div class="ml-2 text-gray-400 font-semibold text-[18px]">{{ user.identities[0].identity_data.full_name }}</div>
						</div>
					</div>
					<div class="relative flex items-center w-full">
						...
						<div class="mt-[10px] bg-gray-600 py-[5px] px-[10px] rounded-lg w-[calc(100%-50px)] text w-full font-light">
							<div class="pt-2 text-gray-200 bg-gray-600 w-full">
								<textarea
									v-model="text"
									style="resize: none"
									...
									class="w-full bg-gray-600 outline-none"
								></textarea>
							</div>
			...
			<button
				...
				:disabled="isLoading"
				@click="createPost"
				class="fixed bottom-0 font-bold text-lg w-full p-2 bg-gray-600 inline-block float-right p-4 border-t border-t-gray-700"
				:class="isLoading ? 'text-gray-300' : 'text-gray-300'"
			>
...
<script setup>
	...
	const createPost = async () => {
		...
		if (fileData.value) {
			const { data, error } = await client.storage.from("polylogue_files").upload(`${uuidv4()}.jpg`, fileData.value);
