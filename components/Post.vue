		...
		<div class="py-2 w-full">
			<div class="flex items-center justify-between">
				<div class="flex items-center text-white">
					...
					<div class="ml-2 font-semibold text-[16px] text-gray-600">{{ post.name }}</div>
				</div>
				<div
			...
			<div class="relative flex items-center w-full">
				<div class="w-[42px] mx-auto"></div>
				<div class="bg-gray-700 rounded-lg w-[calc(100%-50px)] text-sm w-full font-light">
					<div class="p-3 text-gray-400 font-medium">{{ post.text }}</div>
					<img
					...
					<div class="flex items-center absolute mt-2 w-full gap-2">
...
<script setup>
	...
	const deletePost = async (id, picture) => {
		try {
			...
			const { data, error } = await client.storage.from("polylogue_files").remove([picture]);
